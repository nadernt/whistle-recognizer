...
{
	int fft_len=FFT_LEN/2,i=StartPointOfListen;
...
void FAR PASCAL OnTimerShot(UINT wTimerID, UINT msg, DWORD dwUser, DWORD dw1, DWORD dw2)
{
		...
		//AfxMessageBox("Learne is ended.2");
}
...
void Listener::LearningSampleSound(int CUT_OFF_OFFSET,bool InternalLearn)
{
	SndCard Snd;
	...
	float tmp1=0,tmp2=0;
/*	if (Snd.nMuteGetState("OUT_SPEAKERS")==0)
	{
		OUT_SPEAKERS=TRUE;
	}
			Snd.nMuteUnMute("OUT_SPEAKERS",0);*/
...
//		Snd.nMuteUnMute("OUT_SPEAKERS",OUT_SPEAKERS);
		//double tmpWeight=0;
		...
		CString Tito;
		Tito.Format("%f %d ",AvrgWeightTotal,ito);
		Tito+=" Learn is ended.";
		AfxMessageBox(Tito);
}
